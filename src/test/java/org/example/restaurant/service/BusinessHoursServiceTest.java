...
class BusinessHoursServiceTest {
    ...
    @Nested
    @DisplayName("Open Status Checking")
    class OpenStatusChecking {
        @Test
    ...
    @Nested
    @DisplayName("Restaurant Filtering")
    class RestaurantFiltering {
        @Test
    ...
    @Nested
    @DisplayName("Next Opening Calculation")
    class NextOpeningCalculation {
        @Test
        ...
        @Test
        @DisplayName("getNextOpenTime - 14 天內都不營業返回 null")
        void getNextOpenTime_NoOpeningIn14Days_ReturnsNull() {
            Restaurant restaurant = new Restaurant("1", "Test");
            restaurant.setActive(true);
            BusinessHours hours = new BusinessHours();
            hours.setClosedOnHolidays(true);
            // 不設定任何營業時間
            restaurant.setBusinessHours(hours);
            LocalDateTime from = LocalDateTime.now();
            assertNull(service.getNextOpenTime(restaurant, from));
        }
    }

    @Nested
    @DisplayName("Closing Time Calculation")
    class ClosingTimeCalculation {
        @Test
    ...
    @Nested
    @DisplayName("Closing Soon Detection")
    class ClosingSoonDetection {
        @Test
        ...
        @Test
        @DisplayName("isClosingSoon - 餐廳未營業返回 false")
        void isClosingSoon_NotOpenNow_ReturnsFalse() {
            // 創建一個只在早上營業的餐廳
            Restaurant restaurant = new Restaurant("1", "Test");
            restaurant.setActive(true);
            BusinessHours hours = new BusinessHours();
            // 設定只在 6:00-7:00 營業，現在肯定不會是這個時間
            hours.setHours(DayOfWeek.MONDAY, LocalTime.of(6, 0), LocalTime.of(7, 0));
            hours.setHours(DayOfWeek.TUESDAY, LocalTime.of(6, 0), LocalTime.of(7, 0));
            hours.setHours(DayOfWeek.WEDNESDAY, LocalTime.of(6, 0), LocalTime.of(7, 0));
            hours.setHours(DayOfWeek.THURSDAY, LocalTime.of(6, 0), LocalTime.of(7, 0));
            hours.setHours(DayOfWeek.FRIDAY, LocalTime.of(6, 0), LocalTime.of(7, 0));
            hours.setHours(DayOfWeek.SATURDAY, LocalTime.of(6, 0), LocalTime.of(7, 0));
            hours.setHours(DayOfWeek.SUNDAY, LocalTime.of(6, 0), LocalTime.of(7, 0));
            restaurant.setBusinessHours(hours);

            // 假設現在不是 6:00-7:00，餐廳未營業
            assertFalse(service.isClosingSoon(restaurant, 30));
        }

        @Test
        @DisplayName("isClosingSoon - 無營業時間返回 false")
        void isClosingSoon_NoBusinessHours_ReturnsFalse() {
            Restaurant restaurant = new Restaurant("1", "Test");
            restaurant.setActive(true);
            // 沒有設定營業時間
            assertFalse(service.isClosingSoon(restaurant, 30));
        ...
        @Test
        @DisplayName("isClosingSoon - 今日無營業時間返回 false")
        void isClosingSoon_TodayNoHours_ReturnsFalse() {
            Restaurant restaurant = new Restaurant("1", "Test");
            restaurant.setActive(true);
            BusinessHours hours = new BusinessHours();
            // 不設定今日的營業時間
        ...
        @Test
        @DisplayName("isClosingSoon - 非活躍餐廳返回 false")
        void isClosingSoon_InactiveRestaurant_ReturnsFalse() {
            Restaurant restaurant = createRestaurantWithHours(LocalTime.of(0, 0), LocalTime.of(23, 59));
            restaurant.setActive(false);
            assertFalse(service.isClosingSoon(restaurant, 30));
        }
        ...
        @Test
        @DisplayName("isClosingSoon - 24小時營業餐廳測試")
        void isClosingSoon_24HoursRestaurant_Test() {
            Restaurant restaurant = createRestaurantWithHours(LocalTime.of(0, 0), LocalTime.of(23, 59));
            // 24 小時營業的餐廳，即將在 23:59 關閉
            // 這取決於當前時間，結果可能為 true 或 false
            boolean result = service.isClosingSoon(restaurant, 30);
            // 不論結果如何，確保沒有拋出異常
            assertNotNull(Boolean.valueOf(result));
        }

        @Test
        @DisplayName("isClosingSoon - 跨夜營業餐廳（關店時間在開店時間之前）")
        void isClosingSoon_OvernightHours_Test() {
            Restaurant restaurant = new Restaurant("1", "Test");
            restaurant.setActive(true);
            BusinessHours hours = new BusinessHours();
            // 設定跨夜營業 22:00 - 02:00
            for (DayOfWeek day : DayOfWeek.values()) {
                hours.setHours(day, LocalTime.of(22, 0), LocalTime.of(2, 0));
            }
            restaurant.setBusinessHours(hours);
            // 跨夜營業時，closingTime.isBefore(now) 會為 true
            // 所以應該返回 false
            boolean result = service.isClosingSoon(restaurant, 30);
            assertNotNull(Boolean.valueOf(result));
        }
        ...
        @Test
        @DisplayName("findClosingSoon - 過濾 null 餐廳")
        void findClosingSoon_FiltersNullRestaurants() {
            Restaurant restaurant = createRestaurantWithHours(LocalTime.of(0, 0), LocalTime.of(23, 59));
            List<Restaurant> list = Arrays.asList(restaurant, null);
            List<Restaurant> result = service.findClosingSoon(list, 30);
            // 不應該拋出異常
